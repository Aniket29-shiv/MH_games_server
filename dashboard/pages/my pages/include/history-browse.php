<?php
    ...
    function totalpages($limit,$sql,$searchval,$from,$to) {
        ...
        $makequery="SELECT l.user_id,u.* FROM `browse_history` as l left join users as u on u.user_id=l.user_id where u.username !='' ";
        if($searchval != ''){ $makequery .=" and  ( l.`username` like '$searchval%' or l.`email` like '$searchval%')"; }
        if($from != ''){ $makequery .=" and  l.`created_date` >= '$from 00:00:00'"; }
        if($to != ''){ $makequery .=" and  l.`created_date` <= '$to 23:59:59'"; }
        $makequery .=" group by l.user_id";
...
function listpromotions($sql,$start,$limit,$searchval,$from,$to){
    ...
    $makequery="SELECT l.user_id,u.* FROM `browse_history` as l left join users as u on u.user_id=l.user_id where u.username !=''  ";
        ...
        if($from != ''){ $makequery .=" and  l.`created_date` >= '$from 00:00:00'"; }
        if($to != ''){ $makequery .=" and  l.`created_date` <= '$to 23:59:59'"; }
        $makequery .=" group by l.user_id LIMIT $start,$limit";
    ...
    if(mysqli_num_rows(	$query) > 0){     
    	...
    	while($listdata=mysqli_fetch_object($query)){
               ...
               <td>'.$listdata->username.'</td>
                ...
                <td><a href="browse-history-details.php?userid='.$listdata->user_id.'" class="btn btn-primary">Browse History</a></td>
                 </td>
                </tr> ';
    	    $x++;
