...
function totalpages($limit,$conn,$joker,$players,$bet,$status) {
	
	
    $makequery="select * from player_table where game='Cash Game' AND game_type='Deal Rummy' ";
...
$tpages = totalpages(10,$sql,$joker,$players,$bet,$status);
    ...
    if(mysqli_num_rows(	$query) > 0){     
		...
		echo ')" href="?did='.$listdata->table_id.'"><i class="glyphicon glyphicon-trash" title="Delete" aria-hidden="true"></i></a></td>';
              /*  if($listdata->table_status=='S'){ 
                ...
                echo '<a href="#"> <button class="btn btn-default">Stop</button></a> <a href="table_action.php?live='.$listdata->table_id.'&page=dash-deal-lobby-rummy.php"><button class="btn btn-success">Live</button> </a>';
                
                }else{ 
                    
                echo '<a href="table_action.php?stop='.$listdata->table_id.'&page=dash-deal-lobby-rummy.php"> <button class="btn btn-danger">Stop</button></a><a href="#"><button class="btn btn-default">Live</button></a>';
    ...
    if(isset($_GET['did'])){
        ...
        if($did != ''){
         ...
         echo '<script>alert("Table  Deleted Successfully");</script>';
          echo '<script>window.location.href="dash-deal-lobby-rummy.php"</script>';
        }
    }
?>
