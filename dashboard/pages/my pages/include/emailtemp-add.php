...
if(isset($_GET['did'])){
    $did=$_GET['did'];
    mysqli_query ($conn,"update `email-template` set deleted=1 where id='$did'");
...
if(isset($_POST['btnSubmit'])){
        ...
        if($subject == ''){$reg=1; $message="Subject Required";}
        if($emessage == ''){$reg=1; $message="Message Required";}
          ...
          if($reg == 0){
             ...
             if($eid == ''){
                $makequery="INSERT INTO `email-template`(`title`, `subject`, `emessage`, `created_date`) VALUES ('$title','$subject', '$emessage', '$cdate')";
                	mysqli_query ($conn,$makequery);
            	echo '<script>window.location.href="email-template.php?status=1"</script>';
             }else{
                 $makequery="UPDATE `email-template` SET `title`='$title',`subject`='$subject', `emessage`='$emessage' where id='$eid'";
                	mysqli_query ($conn,$makequery);
            	echo '<script>window.location.href="email-template.php?status=2"</script>';
