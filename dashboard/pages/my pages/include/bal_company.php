...
function totalpages($limit,$sql,$game,$players,$from,$to,$chiptype,$status) {
    ...
    if($status != ''){ $makequery .=" and `player_capacity`='$status'"; }
...
function listpromotions($sql,$start,$limit,$game,$players,$from,$to,$chiptype,$status){
        ...
        if($game != ''){ $makequery .=" and `game_type`='$game'"; }
        if($status != ''){ $makequery .=" and `player_capacity`='$status'"; }
        if($players != ''){ $makequery .=" and (`players_name` like '%$players%')"; }
        if($from != ''){ $makequery .=" and  `date` >= '$from 00:00:00'"; }
        if($to != ''){ $makequery .=" and  `date` <= '$to 23:59:59'"; }
    ...
    if(mysqli_num_rows(	$query) > 0){   
    	...
    	while($listdata=mysqli_fetch_object($query)){
                    ...
                    <td>'.number_format($listdata->total_amount,2).'</td> 
...
function totalwin($sql,$game,$players,$from,$to,$chiptype,$status){
      ...
      $makequery="SELECT sum(amount) as total FROM `company_balance`  where 1=1 ";
        if($game != ''){ $makequery .=" and `game_type`='$game'"; }
        if($status != ''){ $makequery .=" and `player_capacity`='$status'"; }
        if($players != ''){ $makequery .=" and (`players_name` like '%$players%')"; }
        if($from != ''){ $makequery .=" and  `date` >= '$from 00:00:00'"; }
