...
function totalpages($limit,$sql,$joker,$players,$bet,$status) {
    ...
    $makequery="select * from player_table where game='Free Game' AND game_type='Pool Rummy' ";
...
function listpromotions($sql,$start,$limit,$joker,$players,$bet,$status){
    ...
    if(mysqli_num_rows(	$query) > 0){     
		...
		echo ')" href="?did='.$listdata->table_id.'"><i class="glyphicon glyphicon-trash" title="Delete" aria-hidden="true"></i></a></td>';
                ...
                echo '<a href="#"> <button class="btn btn-default">Stop</button></a> <a href="table_action.php?live='.$listdata->table_id.'&page=dash-pool-free-rummy.php"><button class="btn btn-success">Live</button> </a>';
                
                }else{ 
                    
                echo '<a href="table_action.php?stop='.$listdata->table_id.'&page=dash-pool-free-rummy.php"> <button class="btn btn-danger">Stop</button></a><a href="#"><button class="btn btn-default">Live</button></a>';
    ...
    if(isset($_GET['did'])){
        ...
        if($did != ''){
         ...
         echo '<script>alert("Table  Deleted Successfully");</script>';
          echo '<script>window.location.href="dash-pool-free-rummy.php"</script>';
        }
    }
?>
