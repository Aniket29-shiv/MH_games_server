<?php
    ...
    function totalpages($limit,$sql,$searchval,$from,$to) {
        ...
        $makequery="SELECT l.userid,u.*,max(l.logindate) FROM `login_history` as l left join users as u on u.user_id=l.userid where 1=1  ";
        ...
        $makequery .=" group by l.userid order by max(l.logindate) desc";
...
function listpromotions($sql,$start,$limit,$searchval,$from,$to){
    ...
    if($from != ''){ $makequery .=" and  l.`logindate` >= '$from 00:00:00'"; }
    if($to != ''){ $makequery .=" and  l.`logindate` <= '$to 23:59:59'"; }
    $makequery .=" group by l.userid order by max(l.logindate) desc LIMIT $start,$limit";
    ...
    if(mysqli_num_rows(	$query) > 0){     
    	...
    	while($listdata=mysqli_fetch_object($query)){
               ...
               <td>'.$listdata->username.'</td>
                ...
                <td><a href="login-history-details.php?userid='.$listdata->userid.'" class="btn btn-primary">Login History</a></td>
                 </td>
                </tr> ';
    	    $x++;
