...
$path = "../../../uploads/mslider/";
$valid_formats = array("jpg", "png", "gif", "bmp","jpeg", "JPG", "PNG", "GIF", "BMP", "JPEG");

if(isset($_GET['did'])){
    $did=$_GET['did'];
    mysqli_query ($conn,"update `mobileslider` set deleted=1 where id='$did'");
    echo '<script>alert("Deleted Successfully")</script>';
    echo '<script>window.location.href="addmslider.php"</script>';
}
...
if(isset($_POST['btnSubmit'])){
    ...
    $makequery="select * from `mobileslider` where deleted != 1 order by rank desc limit 1";
    ...
    $reg=0;
     if($title == ''){$reg=1;$message="Title Required";}
      if($pimage == ''){$reg=1; $message="Image Required";}
       $makequery="INSERT INTO `mobileslider`(`title`, `simage`,`rank`, `created_date`) VALUES ('$title', '$theimage', '$rank', '$cdate')";
       
          if($reg == 0){
             
            	mysqli_query ($conn,$makequery);
            	echo '<script>window.location.href="addmslider.php?status=1"</script>';
          }else{
              echo '<script>window.location.href="addmslider.php?status=1"</script>';
          }
...
function listslider($sql){
    ...
    $makequery="select * from `mobileslider` where deleted != 1";
