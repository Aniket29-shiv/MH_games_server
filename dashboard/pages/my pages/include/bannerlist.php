...
if(isset($_GET['did'])){
    ...
    echo '<script>window.location.href="listbanner.php"</script>';
}
...
function listpromotions($sql,$start,$limit){
    ...
    $makequery="select * from `banners` order by id desc LIMIT $start,$limit";
	...
	while($listdata=mysqli_fetch_object($query)){
            ...
            <td><img src="'.$listdata->bimage.'"  style="width: 100px;" ></td>';
           ...
           echo '<td><a href="addbanner.php?eid='.$listdata->id.'" class="btn btn-primary">Edit</a></td>';
