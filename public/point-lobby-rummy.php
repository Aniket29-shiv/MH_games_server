...
$sql_table= "SELECT * FROM `player_table` WHERE game = 'Cash Game' and `game_type` = 'Point Rummy' and `table_status` = 'L'  group by min_entry,player_capacity order by min_entry ASC";
...
<p id="emptytablemsg" style="color: white; background: green; width: 50%; margin: auto; padding: 10px;display:none;">
    ...
    background: -webkit-linear-gradient(#9b57f0, #4f2f77)" onclick="plus();">	<i  class="glyphicon glyphicon-plus" title="" style="color:#ffffff;"></i></button>
                </div> &nbsp;&nbsp;&nbsp;	<a id="join" target="" onclick="join('<?php echo $loggeduser;?>')"><button  style=" margin-top: 5px;color: #ffffff;
					...
					<div class="col-md-9 game-type over-hid">
						...
						<div class="dropdown black-ft">
					     ...
					     <select class="btn btn-default dropdown-toggle black-ft" id="max" style="color: white;">
					    	...
					    	<section>
						<?php
                    $sql_table1= "SELECT * FROM `player_table` WHERE game = 'Cash Game' and `game_type` = 'Point Rummy' and `table_status` = 'L' group by min_entry,player_capacity order by min_entry ASC ";
...
?>	<div class="table-responsive horizontal">
	<table class="table table-bordered table-striped table-condensed" >
                            	...
                            	if ($result_table1->num_rows > 0)
                            	{
									...
									while($table1 = $result_table1->fetch_assoc())
									{
									  ...
									  $tabl= $table1['table_id'];
											...
											$sql1="select * from  user_tabel_join where `game_type` = 'Point Rummy' and chip_type='real' and player_capacity=".$player_capacity." AND min_entry=".$min_entry."  ";											
									    ...
									    ?>
                                                    ...
                                                    <?php
                                                    }   else if( $table1['player_capacity'] == 6) { ?>
						...
						<!--<section>
						<?php
					// $sql_table1= "SELECT * FROM `player_table` WHERE game = 'Cash Game' and `game_type` = 'Point Rummy' and `table_status` = 'L'  ";
$result_table1 = $conn->query($sql_table1);
?>	<div class="table-responsive horizontal">
	<table class="table table-bordered table-striped table-condensed">
								<thead>
		                   ...
		                   // echo  $a."/".  $table1['player_capacity'];
									?></td>
											...
											<?php //}else if ( $table1['player_capacity'] == 2 ){
											 ?>
											<a id="two_pl_game" onclick="check(<?php //echo $table1['table_id'];?>,'<?php //echo $loggeduser;?>') ;return false;"  target=""><button   class="btn btn-primary">Join</button></a><?php
...
function check_but(a){
    ...
    $("#2_Players").css('background', '#D3D3D3'); 
     ...
     var dataString ='getpointvalue='+pointval;
...
var htmldata=' running table is not available, do you want to seat on new table? <a style="margin: 5px;" onclick="jointableneww();" data-tid="'+table_id+'" data-uid="'+user+'" data-capid="'+player_capacity+'"  class="btn btn-primary" id="tabjoin">Yes</a><a onclick="jointablenewclose();" class="btn btn-danger" id="tabjoincancel">No</a>';
        ...
        function timingLoad() {
        //$('section').load('point-lobby-rummy.php section', function() { });
               var players = $("#players").val();
             	...
             	var logid = $("#logid").val();
        	      ...
        	      var dataString ='players='+players+'&min='+min+'&max='+max+'&hide='+hide+'&logid='+logid;
                    $.ajax({
