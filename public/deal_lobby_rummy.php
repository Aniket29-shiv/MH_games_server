...
$sql_table= "SELECT * FROM `player_table` WHERE game = 'Cash Game' and `game_type` = 'Deal Rummy' and `table_status` = 'L' group by min_entry,player_capacity order by min_entry ASC ";
...
<p id="emptytablemsg" style="color: white; background: green; width: 50%; margin: auto; padding: 10px;display:none;">
			<div class="row black-bg mlr0" style="margin-top: 15px;">
    ...
    <span style="color: white; margin-top: 10px;">Min Entry	 :</span>
              <button class="btn " style=" margin-top: 5px;height: 34px;color: #ffffff;
    ...
    background: -webkit-linear-gradient(#9b57f0, #4f2f77)" id="Deal1"name="Deal1" value="1 Deal" >1 Deal</button>
                  <button type="button" class="btn "  value="2 Deal" style=" margin-top: 5px;color: #ffffff;
    background: -webkit-linear-gradient(#9b57f0, #4f2f77)" id="Deal2" name="Deal2" >2 Deal </button>
							...
							<table class="table table-bordered table-striped table-condensed">
								...
								<?php
									...
									while($table = $result_table->fetch_assoc())
									{
									   $tabl= $table['table_id'];
									    ...
									    $player_capacity= $table['player_capacity'];										  
											...
											$sql1="select * from  user_tabel_join where `game_type` = 'Deal Rummy' and chip_type='real' and player_capacity=".$player_capacity." AND min_entry=".$min_entry."  ";											
									...
									<tr>
										...
										<td>
											<a id="two_pl_game" onclick="check_table('<?php echo $loggeduser;?>','<?php echo $player_capacity;?>','<?php echo $min_entry;?>','<?php echo $game_type;?>','<?php echo $game;?>')"   target=""><button class="btn btn-primary">Join</button></a>
										</td>
									</tr>
								<?php  } } ?>
								</tbody>
								...
								<?php
									...
									<tr>
										...
										<td id="pl_cap"><?php echo  $a."/".  $table['player_capacity'];?></td>
											...
											<a id="two_pl_game" onclick="check(<?php echo $table['table_id'];?>,'<?php echo $loggeduser;?>')"  target=""><button  disabled class="btn btn-primary">Join</button></a>
											<?php }else if ( $table['player_capacity'] == 2 ){
											 ?>
											<a id="two_pl_game" onclick="check(<?php echo $table['table_id'];?>,'<?php echo $loggeduser;?>')"  target=""><button   class="btn btn-primary">Join</button></a><?php
...
function check_but(a){
        ...
        $("#2_Players").css('background', '#D3D3D3'); 
             ...
             var dataString ='getdealvalue='+pointval;
...
function check_table(user,player_capacity,min_entry,game_type,game){
    ...
    var hide = $("#hide").val();
     ...
     var dataString ='dealplayers='+players+'&min='+min+'&max='+max+'&hide='+hide+'&logid='+logid;
