...
$sql_table= "SELECT * FROM `player_table` WHERE game = 'Free Game' and `game_type` = 'Deal Rummy' and `table_status` = 'L' group by min_entry,player_capacity order by min_entry ASC";
...
<p id="emptytablemsg" style="color: white; background: green; width: 50%; margin: auto; padding: 10px;display:none;">
    ...
    background: -webkit-linear-gradient(#9b57f0, #4f2f77)">	<i  class="glyphicon glyphicon-plus" title=""  onclick="plus();"style="color:white;"></i></button>
                ...
                <div class="btn-group" role="group">
                  ...
                  <button type="button" class="btn " onclick="check_but2(this.value);" style=" margin-top: 5px;color: #fff;
    background: -webkit-linear-gradient(#9b57f0, #4f2f77)" id="Deal1"name="Deal1" value="1 Deal" >1 Deal</button>
                  <button type="button" class="btn " onclick="check_but2(this.value);" value="2 Deal" style=" margin-top: 5px;color: #fff;
    background: -webkit-linear-gradient(#9b57f0, #4f2f77)" id="Deal2" name="Deal2" disabled>2 Deal </button>
                 <input type="hidden"  name="btnval2" id="btnval2" value="1 Deal "/>
                </div>
                &nbsp;&nbsp;&nbsp;	<a id="join" target="" onclick="join('<?php echo $loggeduser;?>')"><button  style=" margin-top: 5px;color: #fff;
								...
								<?php
									...
									while($table = $result_table->fetch_assoc())
									{
									     ...
									     $tabl= $table['table_id'];
											...
											$sql1="select * from  user_tabel_join where `game_type` = 'Deal Rummy' and chip_type='free' and player_capacity=".$player_capacity." AND min_entry=".$min_entry."  ";											
											$result_inn = $conn->query($sql1);
											$a = $result_inn->num_rows;	
									?>
									<tr>
										<td style="display:none" id="tbl_id"><?php echo $table['table_id'];?></td>
										...
										<td>
										<a id="two_pl_game" onclick="check_table('<?php echo $loggeduser;?>','<?php echo $player_capacity;?>','<?php echo $point_value;?>','<?php echo $game_type;?>','<?php echo $game;?>','<?php echo $min_entry;?>')"   target=""><button class="btn btn-primary">Join</button></a>
										</td>
									</tr>
								<?php  } } ?>
								</tbody>
							...
							<table class="table table-bordered table-striped table-condensed">
								...
								<?php
									...
									<tr>
										...
										<td id="pl_cap"><?php echo  $a."/".  $table['player_capacity'];?></td>
										<td>
											<?php if( $table['player_capacity'] == 2 && $a == 2 ) { ?>
											<a id="two_pl_game1233" onclick="check(<?php echo $table['table_id'];?>,'<?php echo $loggeduser;?>')"  target=""><button class="btn btn-primary">Join</button></a>
											<?php }else if ( $table['player_capacity'] == 2 ){
											 ?>
											<a id="two_pl_game1233" onclick="check(<?php echo $table['table_id'];?>,'<?php echo $loggeduser;?>')"  target=""><button   class="btn btn-primary">Join</button></a><?php
											} 
											else if( $table['player_capacity'] == 6 && $a == 6) { ?>
											<a id="six_pl_game" target="" onclick="open_six_player_popup(<?php echo $table['table_id'];?>,'<?php echo $loggeduser;?>')"><button  disabled class="btn btn-primary">Join</button></a>
											<?php } else if( $table['player_capacity'] == 6) { ?>
											<a id="six_pl_game" target="" onclick="open_six_player_popup(<?php echo $table['table_id'];?>,'<?php echo $loggeduser;?>')"><button class="btn btn-primary">Join</button></a>
											<?php } ?>
										</td>
									</tr>
								<?php  } } ?>
...
function check_but(a){
      ...
      var dataString ='getdealvaluefree='+pointval;
...
function join(user){
       var pool = $("#btnval2").val(); 
        ...
        var game="Free Game";
        var game_type="Deal Rummy"
// alert(no_of_player+"===="+pool+"=="+game+"=="+game_type+"=="+points);
			$.post("search_deal_player_table.php",
			{
...
function check_table(user,player_capacity,min_entry,game_type,game){
    ...
    var players = $("#players").val();
     	...
     	var hide = $("#hide").val();
          ...
          var dataString ='freedealplayers='+players+'&min='+min+'&max='+max+'&hide='+hide+'&logid='+logid;
           // alert(dataString);
            $.ajax({
    ...
    //$('section').load('deal_rummy_fun.php section', function() {
